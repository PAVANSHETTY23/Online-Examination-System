...
require_once 'sql.php';
    ...
    if (isset($_POST['submit'])) {
        ...
        $op4 = $_POST["op4"];
        $ans = $_POST["ans"];
        $question=$_GET['selector'];
        $quizid = $_GET["qid"];

        //echo '<script>alert('.$quizid.');</script>';
        //echo '<script>alert('.$qs.');</script>';
        //echo '<script>alert('.$op1.');</script>';
        
        //echo '<script>alert('.$op2.');</script>';
        
        //echo '<script>alert('.$op3.');</script>';
        //echo '<script>alert('.$op4.');</script>';


        $sql="UPDATE questions SET questions.qs='$qs', questions.op1='$op1', questions.op2='$op2', questions.op3='$op3', questions.op4='$op4', answer='$ans' WHERE questions.quizid=$quizid AND id=$question;";
        ...
        <section class="dash" style="margin-top:3vw">
            <section id="ans">
                <center>
                    ...
                    $qsid1=$_GET['qid'];
                    $id=$_GET['selector'];
                    $sql ="select * from questions where quizid=$qsid1 AND id=$id";
                    $res=mysqli_query($conn,$sql);
                    //if($res)
                    //{
                        ...
                        <div id="QS">
                            ...
                            <br><label><?php echo $row["qs"];?></label><br><label for="qs">Question:</label>
                            <input type="text" name="qs" placeholder="enter question "  required><br><br>
                            
                            <label for="op1">Option 1</label>
                            <input type="text" name="op1" placeholder="option1" value=<?php echo $row["op1"];?> required><br><br>
                            
                            <label for="op2">Option 2</label>
                            <input type="text" name="op2" placeholder="option2" value=<?php echo $row["op2"];?> required><br><br>
                            
                            <label for="op3">Option 3</label>
                            <input type="text" name="op3" placeholder="option3" value=<?php echo $row["op3"];?> required><br><br>
                            <label for="op4">Option 4</label>
                            <input type="text" name="op4" placeholder="option4" value=<?php echo $row["op4"];?> required><br><br>
                            <label for="ans">Correct Option &nbsp;</label>
                            <input type="text" name="ans" placeholder="answer" value=<?php echo $row["answer"];?> required><br><br>
                        </div>
                        <input type="submit" name="submit" value="Done" style="height: 3vw;width: auto;font-family: 'Courier New', Courier, monospace;font-weight: bolder;border-radius: 10px;border: 2px solid black;background-color: lightblue;">
